...
**以下内容最后更新于 2019/09/02**

## 1.8 版本更新相关
### 星球视觉效果更新
<div id="v1">
<iframe src="//player.bilibili.com/player.html?aid=63178736&cid=109725107&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" style="width:100%;height:100%;"> </iframe>
</div>
<script>
    // Credit: https://blog.asucreyau.xyz/2018/11/19/bilibili-embed-optimize/
    // 加载视频的时候适配
    change();
    // 监听浏览器的resize事件，随时适配
    ...
    function change() {
        var c = document.getElementById("v1");
        var p = c.parentNode;   // div容器的父节点，p是parent的首字母
        // 下面是适配容器的宽高
        c.style.width = p.offsetWidth * 0.9 + "px"; // 适配博客页面乘0.9
...
<div id="v2">
<iframe src="//player.bilibili.com/player.html?aid=63845662&cid=110870485&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" style="width:100%;height:100%;"> </iframe>
</div>
<script>
    change();
    window.onload = function() {
    ...
    function change() {
        ...
        var p = c.parentNode;
        c.style.width = p.offsetWidth * 0.9 + "px";
        if (navigator.userAgent.indexOf('iPad') === -1 && p.offsetWidth >= 480) {
        c.style.height = c.offsetWidth / 16 * 9 + 118 + 'px';
        } else {
        c.style.height = c.offsetWidth / 16 * 9 + 'px';
...
风扇叶片与外壳  
![image](/img/posts/tieba-FAQ-1908-01.png)  
...
- 若电脑具备较大显存的独立显卡，可使用 DX11/DX12/OpenGL 启动游戏，参考 [《DX11: KSP 内存拯救者》](https://duck1998.github.io/2018/02/15/ksp-with-dx11-to-save-ram/)
- 加大电脑内存
...
### 降级游戏版本
参考 [《坎巴拉版本回滚计划》](https://duck1998.github.io/2018/03/10/rollback-ksp-and-repair-saves/) 一文。  
DLC 对游戏版本要求：
...
- 8GB 内存，安装大量组件/美化类 MOD 则 16GB
- 独立显卡，[使用 DX11 启动游戏](https://duck1998.github.io/2018/02/15/ksp-with-dx11-to-save-ram/) 可减少内存占用
...
### 原版星系 Δv 图表
![image](/img/posts/tieba-FAQ-1908-02.png)  
...
3. 点击编辑器左上角「**工具：变更根组件**」按钮，或按快捷键「**4**」
...
### 「同个航天器互动」的含义
<div id="v3">
    <iframe src="//player.bilibili.com/player.html?aid=57905707&cid=101052758&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true" style="width:100%;height:100%;"></iframe>
</div>
<script>
    change();
    window.onload = function() {
...
更多详情参考 [《DX11: KSP 内存拯救者》](https://duck1998.github.io/2018/02/15/ksp-with-dx11-to-save-ram/) 一文。
...
#### RSS Δv 图表
![image](/img/posts/tieba-FAQ-1908-03.png)  
